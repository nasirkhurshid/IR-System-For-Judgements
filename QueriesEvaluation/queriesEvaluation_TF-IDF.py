...
for topCats in docsCategories:
    ...
    line = ('Query Term-'+ str(index+1)).ljust(18)+'TF-IDF'.ljust(12)+str(precisionVal).ljust(6)+str(recallVal).ljust(6)+str(f1_score).ljust(6)+str(round(final_average_percision,3)).ljust(6)+'\n'
    lines.append(line)
    index+=1
mean_average_percision = mean_average_percision/10

with open('tfidfEvaluation.txt', 'w') as f:
...
print('\nEvaluation Data written to tfidfEvaluation.txt\n')